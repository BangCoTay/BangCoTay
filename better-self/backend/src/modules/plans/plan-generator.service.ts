...
export class PlanGeneratorService {
  ...
  private getDifficulty(day: number): 'beginner' | 'intermediate' | 'advanced' {
  ...
  async generatePlan(
    ...
    try {
      ...
      const { data: existingPlan } = await this.supabaseService
        .getAdminClient()
        .from('plans')
        .select('id')
        .eq('user_id', userId)
      ...
      if (planError || !plan) {
        this.logger.error(`Plan creation failed: ${planError?.message}`);
        throw new Error('Failed to create plan');
      }

      // Determine how many days to unlock based on subscription
      const limits =
        SUBSCRIPTION_LIMITS[subscriptionTier] || SUBSCRIPTION_LIMITS.free;
      const daysToUnlock = limits.daysUnlocked;

      // Generate tasks for all 30 days
      const quitTasksSource =
        QUIT_TASKS[onboardingData.addiction] || QUIT_TASKS.default;
      const adoptTasksSource =
        onboardingData.healthy_habit !== HealthyHabit.NONE
          ? ADOPT_TASKS[onboardingData.healthy_habit]
          : null;
      ...
      for (let day = 1; day <= 30; day++) {
        const difficulty = this.getDifficulty(day);
        const unlocked = day <= daysToUnlock;

        // Create day plan
        const { data: dayPlan, error: dayPlanError } =
          await this.supabaseService
            .getAdminClient()
            .from('day_plans')
            .insert({
        ...
        if (dayPlanError || !dayPlan) {
          this.logger.error(
            `Day plan creation failed: ${dayPlanError?.message}`,
          );
          continue;
        }

        // Generate quit task
        const availableQuitTasks = quitTasksSource[difficulty].filter(
          (t) => !usedQuitTasks.has(t),
        );
        const quitTask =
          availableQuitTasks.length > 0
            ? availableQuitTasks[
                Math.floor(Math.random() * availableQuitTasks.length)
              ]
            : quitTasksSource[difficulty][
                Math.floor(Math.random() * quitTasksSource[difficulty].length)
              ];
        ...
        // Generate adopt task
        if (adoptTasksSource) {
          const availableAdoptTasks = adoptTasksSource[difficulty].filter(
            (t) => !usedAdoptTasks.has(t),
          );
          const adoptTask =
            availableAdoptTasks.length > 0
              ? availableAdoptTasks[
                  Math.floor(Math.random() * availableAdoptTasks.length)
                ]
              : adoptTasksSource[difficulty][
                  Math.floor(
                    Math.random() * adoptTasksSource[difficulty].length,
                  )
                ];

          usedAdoptTasks.add(adoptTask);

          await this.supabaseService.getAdminClient().from('tasks').insert({
            day_plan_id: dayPlan.id,
            user_id: userId,
            task_order: 2,
            task_type: TaskType.ADOPT,
            title: adoptTask,
            description: 'Build your new habit',
            completed: false,
          });
        } else {
          // Self-care task if no healthy habit
          const selfCareTasks = [
