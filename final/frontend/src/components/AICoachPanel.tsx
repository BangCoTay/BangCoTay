...
export function AICoachPanel({ onUpgrade }: AICoachPanelProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-card rounded-2xl border overflow-hidden min-h-0">
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center space-y-2">
              ...
              {selectedPersona === "coach" ? (
                ...
                <p className="text-sm text-muted-foreground">
                  Messages from {currentPersona.name} will appear here when you
                  complete tasks
                </p>
              )}
            </div>
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className={cn(
                ...
                (!canSendMessage || sendMessage.isPending) && "opacity-50",
              )}
            />
            <button
              onClick={handleSend}
              disabled={
                sendMessage.isPending || (canSendMessage && !inputValue.trim())
              }
              className={cn(
                "p-3 rounded-xl transition-all flex items-center gap-2 min-w-[44px]",
                canSendMessage
                  ? inputValue.trim() && !sendMessage.isPending
                    ? "bg-primary text-primary-foreground hover:opacity-90 shadow-lg"
                    : "bg-muted text-muted-foreground cursor-not-allowed"
                  : "bg-gradient-to-r from-primary to-accent text-primary-foreground hover:opacity-90 shadow-lg px-4",
              )}
            >
              {sendMessage.isPending ? (
