import type { OnboardingData, Quote, Niche } from '@/types';

const QUOTES: Record<Niche | 'general', { emotional: string[]; practical: string[] }> = {
  digital: {
    emotional: [
  ...
  gaming: {
    emotional: [
      ...
      "The achievements that matter can't be unlocked in a game.",
    ...
    practical: [
      ...
      "Track your gaming hours for a week. The numbers don't lie.",
...
function getRandomQuotes(arr: string[], count: number, exclude: Set<string> = new Set()): string[] {
  const available = arr.filter(q => !exclude.has(q));
  const shuffled = available.sort(() => Math.random() - 0.5);
  return shuffled.slice(0, count);
}

export function generateQuotes(data: OnboardingData): Quote[] {
  const { niche } = data;
  ...
  const nicheEmotional = getRandomQuotes(source.emotional, 1);
  const generalEmotional = getRandomQuotes(generalSource.emotional, 1, new Set(nicheEmotional));
  
  // Get 1 practical quote
  const practical = getRandomQuotes(source.practical, 1);
  
  [...nicheEmotional, ...generalEmotional].forEach((text, i) => {
    quotes.push({
      id: `quote-emotional-${i}`,
      text,
      category: 'emotional',
    });
  });
  
  practical.forEach((text, i) => {
    quotes.push({
      id: `quote-practical-${i}`,
      text,
      category: 'practical',
    });
  });
