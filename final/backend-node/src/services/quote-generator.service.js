...
const getRandomQuotes = (arr, count, exclude = new Set()) => {
  const available = arr.filter((q) => !exclude.has(q));
  const shuffled = [...available].sort(() => Math.random() - 0.5);
  return shuffled.slice(0, count);
};

const generateQuotes = (niche) => {
  ...
  [...nicheEmotional, ...generalEmotional].forEach((text) => {
    quotes.push({ text, category: 'emotional', niche: niche || 'general' });
  });

  practical.forEach((text) => {
    quotes.push({ text, category: 'practical', niche: niche || 'general' });
  });
